...
def fill_db(servicein):
    path = Path(__file__).parent.parent / "utils" / "users.json"
    with open(path) as f:
        users = json.load(f)
    api_users = []
    for user in users:
        resp = servicein.create_user(user)
        api_users.append(resp.json())

    yield api_users

    for user in api_users:
        resp = servicein.delete_user(user["id"])


@pytest.fixture
def users_from_db(servicein):

    def get_users():
        resp = servicein.get_users()
...
class TestUsers:
    ...
    def test_create_user(self, user, servicein):
        payload = UserCreate(**user).model_dump(mode="json")
        resp = servicein.create_user(payload)
        resp_data = resp.json()
    ...
    def test_create_user_invalid_data(self, user, servicein):
        resp = servicein.create_user(user)
        assert resp.status_code == HTTP_422_UNPROCESSABLE_ENTITY

    @pytest.mark.parametrize('user_id, expected_code', id_with_status)
    def test_get_user(self, servicein, user_id, expected_code):
        resp = servicein.get_user(user_id)
        assert resp.status_code == expected_code

    @pytest.mark.parametrize('user_id', user_ids['exist'])
    def test_delete_existed_user(self, user_id, servicein):
        resp = servicein.delete_user(user_id)
        assert resp.status_code == HTTP_204_NO_CONTENT

    @pytest.mark.parametrize('user_id', user_ids['not_exist'])
    def test_delete_not_existed_user(self, user_id, servicein):
        resp = servicein.delete_user(user_id)
        assert resp.status_code == HTTP_404_NOT_FOUND

    def test_delete_invalid_method(self, servicein):
        url = servicein.session.base_url + f"users/{user_ids['exist'][0]}"
        resp = servicein.session.request("POST", url)
        assert resp.status_code == HTTP_405_METHOD_NOT_ALLOWED

    @pytest.mark.parametrize('user_id, user_up', updated_users)
    def test_update_existed_user(self, user_id, user_up, servicein):
        payload = UserPatch(**user_up).model_dump(mode="json")
        resp = servicein.update_user(user_id, payload)
        assert resp.status_code == HTTP_200_OK
        resp_data = resp.json()
    ...
    def test_get_all_users(self, servicein):
        resp = servicein.get_users()
        assert resp.status_code == HTTP_200_OK
...
class TestSmoke:
    def test_check_status(self, servicein):
        resp = servicein.check_status()
        assert resp.status_code == HTTP_200_OK

    def test_check_status_users(self, servicein):
        resp = servicein.check_status()
        assert resp.status_code == HTTP_200_OK
